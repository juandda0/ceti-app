...
export default function CetisDisplay({ amount, showLabel = true }: CetisDisplayProps) {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.row}>
        ...
        <Text
          style={[styles.number, Typography.displayNumberMedium, { color: colors.gold.primary }]}
        >
          {amount}
        </Text>
        {showLabel && (
          <Text style={[styles.label, Typography.title3, { color: colors.text.secondary }]}>
            Cetis
          </Text>
        )}
      </View>
      ...
      {diff !== 0 && (
        <Animated.View
          key={`${amount}-${diff}`}
          entering={motion.enterUp(28)}
          exiting={motion.exitUp}
